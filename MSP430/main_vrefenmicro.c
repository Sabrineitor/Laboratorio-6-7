// (Conf. ADC10) Sacamos REF2_5V y REFON, agregamos: REFOUT_1 (Que es lo que nos permite activar el voltaje de referencia)
// Pasamos la led al P3
// Pasamos el ADC10 al pin 3 (Cuidado que en la interrupción tambien se cambia)
// Le agregamos un __delay_cycles(100000000); antes de la confg. del ADC10 (Esperar 12,5 seg. a 8MHz)
// Cambiamos CONSEQ a 2 y agregamos MSC_1 en la confg. del ADC10 (Creemos que eso habilita la toma de datos continua)
...
{ 
  ...
  __delay_cycles(100000000);//Creemos que estamos esperando 12,5 seg (8MHz)

  //setup the ADC10
  ADC10CTL0 &= ~ENC;				// Disable Conversion (apagar antes de configurar)
  ADC10CTL0 = SREF_1 + REFOUT_1 + ADC10SHT_0 + ADC10ON + ADC10IE + ADC10SR + ADC10SC + MSC_1; // Rango, sample and hold time, ADC10ON, interrupt enabled
  ADC10AE0 |=  BIT1;     //  El canal donde grabamos por lo que vimos es la patita 4
